...
const DigitalAddressVerification = () => {
    ...
    const isApplicationExists = useCallback(() => {
        ...
        if (
            ...
            fetch(
                `http://bgvadmin.goldquestglobal.in:5000branch//candidate-application/digital-address-verification/is-application-exist?app_id=${decodedValues.app_id}&branch_id=${decodedValues.branch_id}&customer_id=${decodedValues.customer_id}`
            )
                .then(res => res.json())
                .then(result => {
                    if (!result.status) {
    ...
    const uploadCustomerLogo = async (candidate_application_id, branch_id, customer_id) => {
        for (const [key, value] of Object.entries(files)) {
            ...
            try {
                const response = await axios.post(`http://bgvadmin.goldquestglobal.in:5000/branch/candidate-application/digital-address-verification/upload`, customerLogoFormData, {
    ...
    const handleSubmitForm = async (e) => {
        ...
        try {
            const response = await fetch(
                "http://bgvadmin.goldquestglobal.in:5000/branch/candidate-application/digital-address-verification/submit",
                requestOptions
            );

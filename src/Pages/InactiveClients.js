...
const InactiveClients = () => {
  ...
  const fetchClients = useCallback(async () => {
    ...
    try {
      const response = await fetch(`http://bgvadmin.goldquestglobal.in:5000/customer/inactive-list?admin_id=${admin_id}&_token=${storedToken}`);
  ...
  const inActive = async (name, id) => {
    ...
    if (confirm.isConfirmed) {
      ...
      try {
        const response = await fetch(`http://bgvadmin.goldquestglobal.in:5000/customer/active?customer_id=${id}&admin_id=${admin_id}&_token=${storedToken}`, { method: 'GET' });

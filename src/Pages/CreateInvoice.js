...
const CreateInvoice = () => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(
      `https://api.goldquestglobal.in/generate-invoice?${queryString}`,
      requestOptions
    )
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        ...
        if (applications.length > 0) {
          const overallServiceAmountNumber = !isNaN(
            Number(overallServiceAmount)
          )
            ? Number(overallServiceAmount)
            : 0;
          generatePdf(
    ...
    for (const appGroup of applications) {
      for (const application of appGroup.applications) {
        if (application.id === id) {
          const totalAdditionalFee = application.statusDetails.reduce(
            (total, statusDetail) => {
              const fee = parseFloat(statusDetail.additionalFee) || 0;
              return total + fee;
            },
  ...
  const generatePdf = (
    ...
    companyInfoArray.forEach((line, index) => {
      if (line.includes("Website:")) {
        ...
        doc.text(line, pageWidth - rightMargin, yPosition + index * 5, {
          align: "right",
    ...
    const rows1 = serviceInfo.map((service) => {
      const serviceAdditionalFee = getTotalAdditionalFeeByService(
        service.serviceId,
        applications
      );
      const serviceCost = service.totalCost || 0;
      overallServiceAdditionalFeeAmount += serviceAdditionalFee;
      overallServiceAdditionalFeeAmount += serviceCost;

      return [
        service.serviceTitle || "", // Use empty string if null or undefined
        "998521", // Static value remains unchanged
        service.count != null ? service.count.toString() : "", // Check for null or undefined
        service.price != null ? service.price.toString() : "", // Check for null or undefined
        serviceAdditionalFee != null ? serviceAdditionalFee : "", // Use empty string if null/undefined
        ((serviceAdditionalFee || 0) + serviceCost).toString(), // Ensure calculation handles null/undefined values
      ];
    });
    ...
    for (let i = 0; i < bankDetails.length; i++) {
      ...
      doc.rect(
        bankDetailsLeftX,
        startY + i * rowHeight,
        bankLabelColumnWidth,
        rowHeight
      ); // Label cell
      doc.rect(
        bankDetailsLeftX + bankLabelColumnWidth,
        startY + i * rowHeight,
        bankValueColumnWidth,
        rowHeight
      ); // Value cell
      ...
      doc.text(
        value,
        bankDetailsLeftX + bankLabelColumnWidth + 2,
        startY + i * rowHeight + 4
      ); // Value text
    }

    // Tax Details Calculation
    doc.setFontSize(8);
    let newOverallServiceAmount =
      parseInt(overallServiceAmount) +
      parseInt(overallServiceAdditionalFeeAmount);

    const cgstTax = calculatePercentage(
      newOverallServiceAmount,
      parseInt(cgst.percentage)
    );
    const sgstTax = calculatePercentage(
      newOverallServiceAmount,
      parseInt(sgst.percentage)
    );
    const taxDetails = [
      {
        label: "Total Amount Before Tax",
        amount: String(newOverallServiceAmount),
      },
      { label: `Add: CGST - ${cgst.percentage}%`, amount: String(cgstTax) },
      { label: `Add: SGST - ${sgst.percentage}%`, amount: String(sgstTax) },
      {
        label: `Total Tax - ${
          parseInt(cgst.percentage) + parseInt(sgst.percentage)
        }%`,
        amount: String(cgstTax + sgstTax),
      },
      {
        label: "Total Tax Amount (Round off)",
        amount: String(newOverallServiceAmount + cgstTax + sgstTax),
      },
    ...
    doc.setFont("helvetica", "normal");
    const words = wordify(
      parseInt(newOverallServiceAmount + cgstTax + sgstTax)
    );
    ...
    doc.text(annexureText, annexureXPosition, annexureYPosition, {
      align: "center",
    });
    ...
    const rows3 =
      applications[0]?.applications?.length > 0
        ? applications[0].applications.map((app, index) => {
            let totalCost = 0;
            const appAdditionalFee = getTotalAdditionalFee(
              app.id,
              applications
            );
            ...
            const applicationRow = [
              ...
              ...serviceNames.map((service) => {
                ...
                if (serviceExists) {
                  const colPrice =
                    parseInt(getServicePriceById(service.id, serviceInfo)) || 0;
                  service.serviceIndexPrice =
                    (service.serviceIndexPrice || 0) + colPrice;
                  totalCost += colPrice;
                  return colPrice;
                } else {
                  return "NIL";
            ...
            const appCGSTTax =
              calculatePercentage(
                parseInt(totalCost + appAdditionalFee),
                parseInt(cgst.percentage)
              ) || 0;
            const appSGSTTax =
              calculatePercentage(
            ...
            applicationRow.push(
              ...
              appSGSTTax,
              parseInt(
                totalCost + appCGSTTax + appSGSTTax + appAdditionalFee
              ) || 0
            );
            ...
            ); // Use empty string if report_date is not defined
...
Make all your payment Cheques, RTGS/NEFT Payable to: "GOLDQUEST GLOBAL HR SERVICES PRIVATE LIMITED". Payment to be made as per the terms of Agreement. Payments received after the due date shall be liable for interest @ 3% per month, part of month taken as full month. Any discrepancy shall be intimated within 3 working days of receipt of bill. Please email us at accounts@goldquestglobal.com or Contact Us: +91 8754562623.
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text(notesText, leftMargin + 2, boxYPosition + 8, {
      maxWidth: boxWidth - 4,
    });
    ...
    <div className="p-2 md:p-12">
      <div className="bg-white p-3 md:p-12 rounded-md w-full mx-auto">
        ...
        <form onSubmit={handleSubmit} className="">
          ...
          <div>
            <label htmlFor="invoice_number" className="block mb-2">
              Invoice Number:
            </label>
            <input
              type="text"
